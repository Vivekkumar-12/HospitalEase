# Vercel Error Codes (canonical)

This document contains a canonical mapping of Vercel platform error codes to a category and HTTP status code. Conflicts in the original list were resolved by preferring the first occurrence in the source list; conflicts are listed in the "Conflicts" section below.
...
| MICROFRONTENDS_MISSING_FALLBACK_ERROR | Function | 400 |
...
- `FUNCTION_THROTTLED` appeared twice in the original list with different mappings:
  - First occurrence: `Function` / `503` (chosen)
  - Later under "Platform errors": `Internal` / `500`

Resolution: prefer the first (non-Internal) mapping as the canonical entry. If you prefer the Internal mapping instead, I can switch it.


## Usage

- Machine-readable mapping: `errors/vercel-errors.json`
- JS export: `src/vercelErrors.js` (imports the JSON and exports it)
- Quick lookup helpers: `src/errorHandler.js`

If you'd like these placed elsewhere or prefer the Internal `FUNCTION_THROTTLED` mapping, tell me and I'll update accordingly.
